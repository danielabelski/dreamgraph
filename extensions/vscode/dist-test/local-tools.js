...
async function handleRunCommand(input) {
    ...
    const retryTimeoutMs = Math.min(Math.max(Number(input.retryTimeoutMs) || Math.round(baseTimeoutMs * 2), baseTimeoutMs + 1_000), 300_000);
    const out = getOutput();
    ...
    const executeOnce = async (timeoutMs, attempt) => {
        ...
        return new Promise((resolve) => {
            ...
            void vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: `Running: ${command} (attempt ${attempt})`, cancellable: true }, (_progress, token) => {
...
async function executeLocalTool(name, input) {
    ...
    try {
        if (name === 'modify_entity') {
            return handleModifyEntity(args);
        }
        if (name === 'write_file') {
