...
class McpClient {
    ...
    async connect() {
        ...
        this._client = new index_js_1.Client({ name: "dreamgraph-vscode", version: "10.0.0" }, { capabilities: {} });
     ...
     */
    async callTool(name, args = {}, timeoutMs = 300_000, onprogress) {
        this._ensureConnected();
        const result = await this._client.callTool({ name, arguments: args }, undefined, {
            timeout: timeoutMs,
            ...(onprogress
                ? {
                    onprogress: (p) => {
