...
exports.compressToolResult = compressToolResult;
...
function looksLikeServerEnvelope(content) {
...
function reportCompression(coordinator, toolName, originalChars, finalChars, mode) {
    if (!coordinator.recordCompression)
        return;
    try {
        coordinator.recordCompression(`tool:${toolName}`, estimateTokensFromChars(originalChars), estimateTokensFromChars(finalChars), mode);
    }
    catch {
 ...
 */
function compressToolResult(content, coordinator, toolName, options) {
    ...
    if (looksLikeServerEnvelope(content)) {
        reportCompression(coordinator, toolName, originalChars, originalChars, 'envelope-preserved');
        return {
            content,
            originalChars,
    ...
    const expectedSliceTokens = Math.max(1_000, Math.floor(options?.expectedSliceTokens ?? 4_000));
    ...
    if (pressure <= 0.5) {
        ...
        const compressed = `${content.slice(0, headChars)}\n\n` +
    ...
    const targetChars = Math.max(2_000, Math.min(expectedSliceChars, remainingTokens * 4));
...
//# sourceMappingURL=tool-result-compression.js.map