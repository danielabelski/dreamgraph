...
exports.APPLY_LABEL_PATTERN = void 0;
exports.isWriteToolName = isWriteToolName;
exports.isVerifyToolName = isVerifyToolName;
exports.narrowToWriteAndVerify = narrowToWriteAndVerify;
exports.pickPreferredWriteTool = pickPreferredWriteTool;
 ...
 */
function isVerifyToolName(name) {
    if (!name)
        return false;
    const stripped = stripMcpPrefix(name);
    if (stripped === 'read_file')
        return true;
 ...
 */
function narrowToWriteAndVerify(tools) {
 ...
 */
function pickPreferredWriteTool(tools) {
    const PREFERRED = [
    ...
    for (const p of PREFERRED) {
        ...
        for (const t of tools) {
            if (stripMcpPrefix(t.name) === p)
                return t.name;
        }
    }
    // Fallback: first write-classified tool in the catalog.
    for (const t of tools) {
        if (isWriteToolName(t.name))
            return t.name;
    }
    return undefined;
}
 ...
 */
exports.APPLY_LABEL_PATTERN = /\b(apply|patch|implement|write|fix|edit|land|commit|introduce)\b/i;
