...
class ContextBuilder {
    ...
    coordinator) {
        ...
        const selectionText = hasSelection && editor ? editor.document.getText(editor.selection) : null;
        ...
        const envelope = {
            ...
            activeFile: editor
                ? {
                    ...
                    cursorSummary: cursorAnchor?.label ??
        ...
        if (envelope.graphContext && envelope.activeFile) {
            if (envelope.activeFile.cursorAnchor) {
                envelope.activeFile.cursorAnchor = await this._promoteAnchor(envelope.activeFile.cursorAnchor, envelope.graphContext);
            ...
            if (envelope.activeFile.selection?.anchor) {
                envelope.activeFile.selection.anchor = await this._promoteAnchor(envelope.activeFile.selection.anchor, envelope.graphContext);
    ...
    async rehydrateStoredAnchors(messages, graphContext) {
        const out = [];
    ...
    async buildReasoningPacket(envelope, options) {
        ...
        const taskPreambleCandidates = included.filter((item) => isGraphEvidenceKind(item.kind) &&
        ...
        const taskPreambleOmissions = omitted
            .filter((entry) => GRAPH_EVIDENCE_KINDS.has(entry.kind))
            .slice(0, 4)
        ...
        const adaptiveFutureJudgment = hasAdaptiveFutureEvidence || taskPreamble.budgetDecision !== 'omit_no_evidence'
            ? {
    ...
    _readPlannedCode(envelope, plan) {
        const editor = vscode.window.activeTextEditor;
        if (!editor)
            return null;
                ...
                ? [
                    {
                        ...
                        anchorLabel: envelope.activeFile.selection?.anchor?.label ??
                            envelope.activeFile.selection?.summary ??
                            envelope.activeFile.cursorAnchor?.label ??
                            envelope.activeFile.cursorSummary,
    ...
    async _resolveGraphContext(envelope, plan, coordinator) {
        ...
        if (envelope.activeFile &&
            ["feature", "workflow", "adr", "ui", "api", "tension"].some((k) => needs.has(k))) {
            const requestBase = {
            ...
            if (result) {
                ...
                const anchor = envelope.activeFile.selection?.anchor ??
                    ...
                    if (targeted) {
                        ...
                        for (const f of targeted.features) {
                            if (existingFeatureIds.has(f.id)) {
                                ...
                                if (existing)
                                    existing.relevance = f.relevance ?? 1.0;
                            }
                            else {
        ...
        settled.forEach((result, index) => {
            ...
            if (kind === "causal") {
                graphCtx.causalChains = result.value;
            }
            else if (kind === "temporal") {
                graphCtx.temporalPatterns = result.value;
            }
            else if (kind === "data_model") {
                graphCtx.dataModelEntities = result.value;
            }
            else if (kind === "cognitive_status") {
                graphCtx.cognitiveState = result.value ?? "unknown";
            }
     ...
     */
    _preScoreFeatureId(anchor, features) {
        ...
        const matchScore = (candidate) => {
            ...
            if (c.startsWith(nameLower) || nameLower.startsWith(c))
                return 0.75;
            if (c.includes(nameLower) || nameLower.includes(c))
                return 0.5;
            return 0;
        };
        let best = null;
    ...
    _applyCanonicalAnchorIdsToGraphContext(envelope) {
        ...
        ].filter((anchor) => Boolean(anchor?.canonicalId));
    ...
    _resolvePrimaryAnchor(envelope) {
        if (envelope.activeFile?.selection?.anchor) {
    ...
    _resolveSecondaryAnchors(envelope, primaryAnchor) {
        const anchors = [];
        if (envelope.activeFile?.cursorAnchor &&
    ...
    _trimActiveFile(fileContent, envelope, remainingBudget) {
        ...
        const activeAnchor = envelope.activeFile?.selection?.anchor ??
            envelope.activeFile?.cursorAnchor;
        const anchorLabel = activeAnchor?.label ??
            envelope.activeFile?.selection?.summary ??
            envelope.activeFile?.cursorSummary ??
        ...
        const minimal = `## Focused Code Excerpt\n` +
    ...
    async _deriveSelectionSemanticAnchor(editor, selectionText) {
    ...
    async _deriveCursorSemanticAnchor(editor) {
        const symbolAnchor = await this._deriveSymbolAnchor(editor, editor.selection.active);
        if (symbolAnchor) {
            return symbolAnchor;
        }
    ...
    async _deriveSymbolAnchor(editor, position, graphContext) {
        try {
    ...
    _findBestSymbolAtPosition(symbols, position, path = []) {
     ...
     */
    async _promoteAnchor(anchor, graphContext) {
        if (!graphContext)
            return anchor;
        const symbolPath = anchor.symbolPath ?? anchor.label ?? "";
        const symbolName = symbolPath.includes(".")
    ...
    _migrateAnchor(anchor, currentSymbols, currentFilePath) {
        ...
        const findByLeafName = (symbols, leafName, path = []) => {
            for (const s of symbols) {
                if (s.name === leafName)
                    return { symbol: s, path: [...path, s] };
     ...
     */
    async resolveAnchorMigration(anchor, graphContext) {
        const editor = vscode.window.activeTextEditor;
        let migrated = anchor;
