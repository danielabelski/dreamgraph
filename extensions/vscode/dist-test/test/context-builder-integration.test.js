...
function collectEvidenceItemsShim(envelope, fileContent, additionalSections, plan) {
    const items = [];
...
(0, node_test_1.default)('shimmed ContextBuilder evidence collection includes environment evidence with stable ordering and compact size', () => {
    ...
    const items = collectEvidenceItemsShim(envelope, 'export function createServer() { return true; }', new Map([['Additional Note', 'low priority note']]), plan);
    ...
    strict_1.default.ok(environmentItem, 'expected environment evidence');
    strict_1.default.ok(featureItem, 'expected feature evidence');
    strict_1.default.ok(noteItem, 'expected note evidence');
    strict_1.default.match(environmentItem.content, /## Environment Context/);
    strict_1.default.match(environmentItem.content, /Scope `src\/server\/`/);
    strict_1.default.match(environmentItem.content, /Framework: MCP server \+ HTTP daemon/);
    strict_1.default.match(environmentItem.content, /Key dependencies: @modelcontextprotocol\/sdk, express, pino/);
    const environmentIndex = items.findIndex((item) => item.kind === 'environment');
    const noteIndex = items.findIndex((item) => item.kind === 'note');
    strict_1.default.ok(environmentIndex >= 0 && noteIndex >= 0 && environmentIndex < noteIndex);
    strict_1.default.ok(environmentItem.tokenCost < 220, `expected compact environment evidence, got ${environmentItem.tokenCost}`);
});
//# sourceMappingURL=context-builder-integration.test.js.map