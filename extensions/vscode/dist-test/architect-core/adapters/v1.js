...
exports.createContextBuilderPort = createContextBuilderPort;
exports.createPromptComposerPort = createPromptComposerPort;
exports.createProviderPort = createProviderPort;
exports.createToolExecutorPort = createToolExecutorPort;
exports.createMemoryPort = createMemoryPort;
exports.createAttachmentPort = createAttachmentPort;
exports.createAutonomyPort = createAutonomyPort;
...
// ---------------------------------------------------------------------------
function createContextBuilderPort(host) {
    return Object.freeze({
        async buildContext(_input) {
            void _input;
            return Object.freeze({
...
// ---------------------------------------------------------------------------
function createPromptComposerPort(host) {
    return Object.freeze({
        async composePrompt(input) {
            const autonomyInstructionState = input.autonomyContract.enabled
                ...
                : undefined;
            const prompt = (0, index_js_1.assemblePrompt)(input.task, 
            // assemblePrompt accepts the v1 envelope shape directly.
            input.contextResult.envelope, input.contextResult.assembledContext, input.additionalInstructions, autonomyInstructionState, input.provider, 
            // Forward the packet's lens object so packet-driven framing
            // continues to work; assemblePrompt accepts it or undefined.
            (input.contextResult.reasoningPacket?.task?.lens));
            ...
            const userTurn = input.userIntent.contentBlocks
                ? { role: "user", content: input.userIntent.contentBlocks }
...
// ---------------------------------------------------------------------------
function createProviderPort(host) {
    return Object.freeze({
        ...
        async callProvider(input) {
            ...
            const wireMessages = [...prompt.conversation];
            if (tools.length === 0) {
                // Pure streaming round-trip. No tool calls possible.
                let buffer = "";
                const streamed = await host.architectLlm.stream(wireMessages, (chunk) => {
                    buffer += chunk;
                    onStreamChunk?.(chunk);
                }, abortSignal);
            ...
            for (const iter of iterationHistory) {
                ...
                if (iter.providerRawAssistant && iter.providerRawAssistant.length > 0) {
                    assistantMsg[openai_responses_adapter_js_1.RESPONSES_RAW_ITEMS_KEY] = iter.providerRawAssistant;
                }
                rawMessages.push(assistantMsg);
                if (iter.toolInvocations.length > 0) {
                    const toolResultBlocks = iter.toolInvocations.map((inv) => ({
                        type: "tool_result",
            ...
            const response = await host.architectLlm.callWithTools(wireMessages, tools.slice(), rawMessages.length > 0 ? rawMessages : undefined, abortSignal);
...
// ---------------------------------------------------------------------------
function createToolExecutorPort(host) {
    return Object.freeze({
...
// ---------------------------------------------------------------------------
function createMemoryPort(host) {
    return Object.freeze({
...
// ---------------------------------------------------------------------------
function createAttachmentPort(host) {
    return Object.freeze({
        buildContentBlocksForTurn(_text) {
            void _text;
...
// ---------------------------------------------------------------------------
function createAutonomyPort(host) {
    return Object.freeze({
