...
if (TARGET_COMMAND.length === 0) {
    process.stderr.write("[copilot-cli-bridge] DREAMGRAPH_BRIDGE_TARGET_COMMAND is required\n");
    process.exit(2);
}
let targetArgs = [];
if (TARGET_ARGS_JSON.length > 0) {
    try {
        ...
        process.stderr.write(`[copilot-cli-bridge] DREAMGRAPH_BRIDGE_TARGET_ARGS_JSON is not a JSON string array: ${err.message}\n`);
        process.exit(2);
    }
}
...
const child = (0, node_child_process_1.spawn)(TARGET_COMMAND, targetArgs, {
    stdio: ["pipe", "pipe", "pipe"],
    windowsHide: true,
});
child.on("error", (err) => {
    process.stderr.write(`[copilot-cli-bridge] failed to spawn target: ${err.message}\n`);
    process.exit(127);
});
// ---------------------------------------------------------------------------
...
const stdoutSniffer = makeNdjsonSniffer((msg) => {
    ...
    let resultJson;
    if ("error" in msg) {
        resultJson = safeStringify(msg.error);
    }
    else {
        resultJson = safeStringify(msg.result ?? null);
    }
    appendAudit({
        server: SERVER_NAME,
 ...
 */
function makeNdjsonSniffer(onMessage) {
    let buf = "";
    return {
        feed(chunk) {
...
function appendAudit(record) {
    if (AUDIT_PATH.length === 0)
        return;
    try {
        (0, node_fs_1.appendFileSync)(AUDIT_PATH, `${JSON.stringify(record)}\n`, { encoding: "utf8" });
    }
    catch (err) {
