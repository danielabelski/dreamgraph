...
exports.TASK_PRESETS = void 0;
exports.getTaskPreset = getTaskPreset;
exports.applyPreset = applyPreset;
exports.presetAutonomyAddendum = presetAutonomyAddendum;
exports.TASK_PRESETS = Object.freeze([
    Object.freeze({
        id: "explain",
        displayName: "Explain",
...
const PRESET_INDEX = new Map(exports.TASK_PRESETS.map((p) => [p.id, p]));
function getTaskPreset(id) {
    const p = PRESET_INDEX.get(id);
    if (!p)
        throw new Error(`Unknown task preset '${id}'`);
    return p;
}
/**
 ...
 * not the user turn.
 */
function applyPreset(intent, presetId) {
    const preset = getTaskPreset(presetId);
    const text = intent.text.trim();
    const wrapped = text.startsWith(preset.userPrefix)
        ? text
        : `${preset.userPrefix}\n${text}`;
    return {
        ...intent,
 ...
 * unconditionally.
 */
function presetAutonomyAddendum(presetId) {
    if (!presetId)
        return "";
