...
exports.DefaultPromptComposer = void 0;
exports.composePromptSync = composePromptSync;
...
class DefaultPromptComposer {
     ...
     */
    declareRequirements(input) {
        return (0, requirements_js_1.declareContextRequirements)(input);
    }
    async composePrompt(input) {
        return composePromptSync(input);
    }
}
exports.DefaultPromptComposer = DefaultPromptComposer;
/**
 ...
 */
function composePromptSync(input) {
    const { contextEnvelope, userIntent, providerProfile, autonomyContract } = input;
 ...
 */
function validateNoActiveSections(envelope) {
    if (envelope.schemaVersion !== 1) {
        throw new Error(`PromptComposer: unsupported ContextEnvelope.schemaVersion=${envelope.schemaVersion}`);
    }
    if (envelope.estimatedTokens > envelope.windowTokens) {
        ...
        throw new Error(`PromptComposer: envelope estimatedTokens=${envelope.estimatedTokens} ` +
...
function renderSystemPrompt(input) {
    ...
    const lines = [];
    lines.push("You are the DreamGraph Architect, an autonomous code-modification agent.");
    lines.push("You operate inside a host editor (VS Code today; other hosts later).");
    lines.push("Your job is to propose ranked next actions as tool calls, with rationale.");
    lines.push("");
    lines.push("## Autonomy contract");
    lines.push(autonomyContract.trim());
    lines.push("");
    lines.push("## Operating principles");
    lines.push("- Prefer one focused action over many. Quality over quantity.");
    lines.push("- Never invent tool names. Choose only from the inventory exposed to you.");
    lines.push("- When proposing a follow-up pass, name the exact next tool you will use.");
    lines.push("- If the context is sparse, say so plainly and propose enrichment as one of your candidates.");
    lines.push("");
    lines.push("## Context provenance");
    lines.push(`Provider: ${providerProfile.id} (${providerProfile.displayName}).`);
    lines.push(`Context envelope: ${contextEnvelope.sections.length} section(s), ` +
        `≈${contextEnvelope.estimatedTokens}/${contextEnvelope.windowTokens} tokens.`);
    lines.push("The context below was assembled by the upstream RelevanceEngine. " +
...
function renderUserPrompt(input) {
    ...
    if (contextEnvelope.sections.length === 0) {
        lines.push("(empty — the RelevanceEngine reported no usable signals for this pass)");
    }
    else {
        ...
        for (const group of GROUP_ORDER) {
            const sections = contextEnvelope.sections.filter((s) => s.source === group);
    ...
    lines.push("Return ranked action candidates with rationale. Use the tool-call shape " +
        "your provider expects. If no action is appropriate, say so explicitly " +
        "and propose pause_for_user.");
    return lines.join("\n");
}
function renderContinuation(c) {
    const lines = [];
    ...
    if (c.alternativesConsidered.length > 0) {
        lines.push(`Alternatives considered: ${c.alternativesConsidered
            .map((a) => a.label)
            .join("; ")}`);
    }
    return lines.join("\n");
 ...
 */
function renderToolContract(profile) {
    switch (profile.id) {
        case "openai":
            return [
