...
import type { Tier, ToolOutcome, ExecutionPlan, FallbackJustification, ArtifactRef, VerificationEvidence } from '../execution/index.js';
...
export type CardKind = 'goal' | 'plan' | 'context' | 'decision' | 'edit' | 'verification' | 'blocker' | 'next-step' | 'completion' | 'fallback' | 'outcome';
...
export type Card = GoalCard | PlanCard | ContextCard | DecisionCard | EditCard | VerificationCard | BlockerCard | NextStepCard | CompletionCard | FallbackCard | OutcomeCard;
/** Exhaustiveness helper — every consumer-side switch must use this. */
export declare function assertNeverCard(c: never): never;
...
export type { Tier, ToolOutcome, ExecutionPlan, FallbackJustification, ArtifactRef, VerificationEvidence, };
