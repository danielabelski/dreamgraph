...
function computeMetrics(input) {
    const { outcomes, capabilityPlans, verificationPlans, classifications, priorRepairAttemptsByKind, enrichmentTasksDrained, } = input;
    // Verifications attempted = sum of steps across plans.
    let verificationsAttempted = 0;
    for (const p of verificationPlans)
        verificationsAttempted += p.steps.length;
    ...
    for (const p of capabilityPlans) {
        if (p.kind === "sparse-mode" && p.enrichmentQueued)
            enrichmentTasksQueued++;
        if (p.kind === "gap") {
            graphOnlyCapabilitiesSkipped++;
            if (p.enrichmentQueued)
                enrichmentTasksQueued++;
        }
    }
...
function aggregateFinalStatus(classifications) {
    if (classifications.length === 0)
        return "completed_no_verification_needed";
    let worst = "completed";
    for (const c of classifications) {
        if (STATUS_RANK[c.kind] > STATUS_RANK[worst])
            worst = c.kind;
    }
    return worst;
}
