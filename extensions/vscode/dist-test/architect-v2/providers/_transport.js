...
exports.DEFAULT_REQUEST_SIZE_LIMIT_BYTES = void 0;
exports.readSseLines = readSseLines;
exports.readNdjsonLines = readNdjsonLines;
exports.serializeWithBudget = serializeWithBudget;
...
// NDJSON (Ollama)
// ---------------------------------------------------------------------------
async function* readNdjsonLines(res) {
    const reader = res.body?.getReader();
    if (!reader) {
        throw new adapter_1.ProviderError({
            kind: "transport",
 ...
 */
exports.DEFAULT_REQUEST_SIZE_LIMIT_BYTES = 320 * 1024;
function serializeWithBudget(providerId, modelId, body, limitBytes) {
    const serialized = JSON.stringify(body);
    const limit = limitBytes ?? exports.DEFAULT_REQUEST_SIZE_LIMIT_BYTES;
    ...
    if (byteLength > limit) {
        throw new adapter_1.ProviderError({
            ...
            providerId,
            modelId,
            message: `Request body ${byteLength} bytes exceeds ${limit}-byte limit. ` +
...
// ---------------------------------------------------------------------------
async function mapHttpError(res, providerId, modelId) {
    let bodyText = "";
    try {
...
// ---------------------------------------------------------------------------
function mapNetworkError(err, providerId, modelId) {
    if (err instanceof adapter_1.ProviderError)
        return err;
    const isAbort = err?.name === "AbortError" ||
        (typeof err === "object" && err !== null && "code" in err && err.code === "ABORT_ERR");
    if (isAbort) {
        return new adapter_1.ProviderError({
            kind: "aborted",
            providerId,
