...
const discovery_recorder_js_1 = require("./discovery-recorder.js");
...
async function assembleContext(input, deps) {
    ...
    for (const { requirement, tokenBudget } of allocations) {
        ...
        if (GRAPH_KINDS.has(requirement.kind)) {
            const graphResult = await tryGraphRequirement(requirement, tokenBudget, deps, input);
            if (graphResult.kind === "ok") {
                ...
                const fallbackResult = await tryFallbackForGraphKind(requirement, tokenBudget, graphResult.fallbackReason, deps, input);
            ...
            const r = await tryFallbackRequirement(requirement, tokenBudget, deps, input);
            if (r.kind === "ok") {
    ...
    if (discoveries.length > 0) {
        const recorder = deps.discoveryRecorder ?? discovery_recorder_js_1.NullContextDiscoveryRecorder;
...
const BUDGET_WEIGHT = Object.freeze({ small: 1, medium: 3, large: 6 });
/**
 ...
 */
function allocateBudget(manifest, windowTokens) {
...
async function tryGraphRequirement(requirement, tokenBudget, deps, input) {
    const richness = await safeCall(() => deps.graphReader.getRichnessSignal({ kind: "global" }), undefined);
    ...
    const subgraph = await safeCall(() => deps.graphReader.retrieveSubgraph({
...
async function tryFallbackForGraphKind(requirement, tokenBudget, fallbackReason, deps, input) {
    ...
    if (requirement.anchors && requirement.anchors.length > 0) {
        const excerpts = await safeCall(() => deps.fallback.readFileExcerpts({
    ...
    if (parts.length === 0) {
        const walk = await safeCall(() => deps.fallback.walkProjectShallow(nodesForBudget(tokenBudget)), []);
        if (walk.length > 0) {
...
// ---------------------------------------------------------------------------
async function tryFallbackRequirement(requirement, tokenBudget, deps, input) {
    switch (requirement.kind) {
        case "environment_state": {
            const env = await safeCall(() => deps.fallback.readEnvironment(), undefined);
            if (!env)
                return { kind: "unmet", reason: "source_error" };
        ...
        case "runtime_diagnostics": {
            const diags = await safeCall(() => deps.fallback.readRecentDiagnostics(itemsForBudget(tokenBudget)), []);
            if (diags.length === 0) {
        ...
        case "recent_history": {
            const hist = await safeCall(() => deps.fallback.readRecentHistory(input.taskState.id, itemsForBudget(tokenBudget)), undefined);
...
// ---------------------------------------------------------------------------
function renderSubgraph(kind, sub) {
    const lines = [];
...
function renderEnvironment(env) {
    ...
    if (env.workspaceName)
        lines.push(`workspace: ${env.workspaceName}`);
    if (env.openFolderUris.length > 0) {
        lines.push(`folders: ${env.openFolderUris.join(", ")}`);
    }
    if (env.activeFileUri)
        lines.push(`active: ${env.activeFileUri}`);
    if (env.extras) {
...
function renderDiagnostics(diags) {
    return diags
        .map((d) => `[${d.severity}] ${d.fileUri}${d.line !== undefined ? `:${d.line}` : ""} ${d.source ? `(${d.source}) ` : ""}${d.message}`)
        .join("\n");
}
function renderHistory(h) {
    const lines = [];
    if (h.taskIntentDigest)
        lines.push(`intent: ${h.taskIntentDigest}`);
...
function renderProjectWalk(entries, fallbackReason) {
...
function collectGraphArtifactRefs(sub) {
    const refs = [];
    for (const n of sub.nodes) {
