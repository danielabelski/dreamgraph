...
import { renderCard, renderPass, renderTrailingNote } from "../../cards/index.js";
import {
  getProvider,
  hasProvider,
  listModelIds,
...
import { VSCodeMementoStore } from "../vscode-memento-store.js";
import { VSCodeFallbackSignalProvider } from "../vscode-fallback-signal-provider.js";
import { V1McpClientBridge, type V1McpClientLike } from "../mcp-client-bridge.js";
import { renderHtml } from "./webview.js";
import type {
  ActivityItem,
  AutonomyModeId,
  HostToWebview,
...
/** Persisted across activations: autonomy mode, provider, per-provider model. */
...
const DEFAULT_MODE: AutonomyModeId = "conscientious";
...
function formatActivityLabel(
  reason: "user_action" | "verification" | "enrichment",
  toolName: string,
): string {
  const verb =
    reason === "verification"
...
function projectTrace(outcomes: readonly ToolOutcome[]): TraceEntry[] {
...
export class ArchitectV2Panel
  implements vscode.WebviewViewProvider, vscode.Disposable
{
  public static readonly viewType = "dreamgraph.architectV2View";

  private readonly context: vscode.ExtensionContext;
  private readonly mcpClient: V1McpClientLike | undefined;

  private view: vscode.WebviewView | null = null;
  private host: ArchitectHost | null = null;
  private hostBuildKey: string | null = null;
  private taskState: TaskState | null = null;
  private mode: AutonomyMode = DEFAULT_MODE;
  private providerId: ProviderId = DEFAULT_PROVIDER;
  ...
  private activityCurrent: ActivityItem | null = null;
  private readonly activityRecent: ActivityItem[] = [];
  private transcript: PersistedTranscriptEntry[] = [];
  private static readonly ACTIVITY_RECENT_LIMIT = 5;
  private static readonly TRANSCRIPT_LIMIT = 200;

  constructor(options: ArchitectV2PanelOptions) {
    this.context = options.context;
    this.mcpClient = options.mcpClient;
    const prefs = this.readPrefs();
    if (prefs.mode && isAutonomyMode(prefs.mode)) this.mode = prefs.mode;
    if (prefs.providerId && hasProvider(prefs.providerId)) {
  ...
  public async resolveWebviewView(
    ...
    webviewView.webview.onDidReceiveMessage(
      (msg: WebviewToHost) => void this.handleMessage(msg),
    );
    ...
    const seedProviders = listProviders().map((p) => ({
      id: p.id,
    ...
    webviewView.webview.html = renderHtml({
      ...
      mode: this.mode as AutonomyModeId,
    });
    ...
    webviewView.onDidChangeVisibility(() => {
      if (webviewView.visible) void this.postInit();
    });
  ...
  private async handleMessage(msg: WebviewToHost): Promise<void> {
    switch (msg.kind) {
      ...
      case "diagnostic":
        ...
        console.warn(
          `[architect-v2] webview diagnostic scope=${msg.scope}: ${msg.sample}`,
        );
        return;
      default: {
  ...
  private async runSubmission(text: string): Promise<void> {
    ...
    try {
      ...
      if (err instanceof UnknownModelError) {
        await this.post({
          kind: "error",
          message:
            `Provider '${err.providerId}' does not advertise model '${err.modelId}'. ` +
    ...
    const userIntent: UserIntent = { text: trimmed };
    this.isRunning = true;
    this.cancelled = false;
    ...
    try {
      ...
      while (true) {
        ...
        const markdown = renderPass({
          cards: result.cards,
          trailingNote: result.trailingNote,
        });
        const chunks: RenderedChunk[] = [];
        for (const card of result.cards) {
          chunks.push({ kind: card.kind, markdown: renderCard(card) });
        }
        const noteMd = renderTrailingNote(result.trailingNote);
        if (noteMd.length > 0) {
        ...
        const wantsContinuation =
          result.newTaskState.status === "running" &&
          !!result.continuation &&
  ...
  private async post(msg: HostToWebview): Promise<void> {
    if (!this.view) return;
    try {
  ...
  private async postInit(): Promise<void> {
    const providers: readonly ProviderOption[] = listProviders().map((p) => ({
      id: p.id,
      displayName: p.displayName,
    }));
    const models = await this.collectModelsForCurrentProvider();
    console.log(
      `[architect-v2] postInit providers=${providers.length} models=${models.length} provider=${this.providerId}`,
    );
    await this.post({
      kind: "init",
      state: {
  ...
  private async postAutonomy(): Promise<void> {
    await this.post({
      kind: "autonomy",
      mode: this.mode,
      passBudget: deriveBudgetView(this.taskState, this.mode),
  ...
  private async ensureHost(apiKey: string): Promise<ArchitectHost> {
    ...
    if (this.host && this.hostBuildKey === buildKey) return this.host;
    const workspaceState = new VSCodeMementoStore(this.context.workspaceState);
    const fallback = new VSCodeFallbackSignalProvider();
    const mcpClient = this.mcpClient
      ? new V1McpClientBridge(this.mcpClient)
      : undefined;
    console.log(
      `[architect-v2] ensureHost provider=${this.providerId} model=${modelId} mcpReady=${mcpReady}`,
    );
    this.host = await createArchitectHost({
      providerId: this.providerId,
  ...
  private buildInitialTaskState(goal: string): TaskState {
    ...
    return {
      ...
      mode: this.mode,
      profile,
      passBudget: createPassBudget(profile.defaultPassBudget),
      timeBudget: createTimeBudget(profile.defaultTimeBudgetMs, now),
      passes: [],
      blockers: [],
      status: "running",
    };
  ...
  private readPrefs(): PersistedPrefs {
    return (
      this.context.workspaceState.get<PersistedPrefs>(PREFS_KEY) ?? {}
    );
  }
  ...
  private readTranscript(): PersistedTranscriptEntry[] {
    const raw = this.context.workspaceState.get<readonly PersistedTranscriptEntry[]>(
      TRANSCRIPT_KEY,
      [],
    );
    return raw.filter((entry): entry is PersistedTranscriptEntry => {
      if (!entry || typeof entry !== "object") return false;
      if (entry.kind === "user-echo") return typeof entry.text === "string";
      return (
        entry.kind === "pass-rendered" &&
  ...
  private async recordTranscriptEntry(
    ...
    if (this.transcript.length > ArchitectV2Panel.TRANSCRIPT_LIMIT) {
      this.transcript.splice(
        0,
        this.transcript.length - ArchitectV2Panel.TRANSCRIPT_LIMIT,
      );
    }
  ...
  private secretKey(providerId: ProviderId = this.providerId): string {
   ...
   */
  private async promptForApiKey(opts: { force: boolean }): Promise<void> {
    if (!opts.force) {
    ...
    const value = await vscode.window.showInputBox({
      ...
      prompt:
        "Stored in VS Code SecretStorage; never written to settings.json. Leave blank to clear.",
      password: true,
      ignoreFocusOut: true,
    });
    if (value === undefined) {
  ...
  private currentModelId(): string {
    ...
    return getProvider(this.providerId).defaultModelId;
  }

  private async collectModelsForCurrentProvider(): Promise<readonly ModelOption[]> {
    try {
  ...
  private async broadcastSettings(): Promise<void> {
    const models = await this.collectModelsForCurrentProvider();
    await this.post({
      kind: "settings",
      hasApiKey: (await this.readApiKey()).length > 0,
  ...
  private async handleSetProvider(providerId: string): Promise<void> {
    if (!hasProvider(providerId)) return;
  ...
  private async handleSetModel(modelId: string): Promise<void> {
    const ids = await listModelIds(this.providerId);
