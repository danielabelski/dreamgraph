...
export interface RenderHtmlInput {
  ...
  readonly providers?: ReadonlyArray<{ id: string; displayName: string }>;
  /** Pre-populated model options for the currently selected provider. */
  readonly models?: ReadonlyArray<{ id: string; displayName: string }>;
...
export function renderHtml(input: RenderHtmlInput): string {
  ...
  const mdUri = webview.asWebviewUri(
    vscode.Uri.joinPath(extensionUri, "dist", "vendor", "markdown-it.min.js"),
  );
  const dpUri = webview.asWebviewUri(
    vscode.Uri.joinPath(extensionUri, "dist", "vendor", "purify.min.js"),
  );
  const csp =
    `default-src 'none'; ` +
