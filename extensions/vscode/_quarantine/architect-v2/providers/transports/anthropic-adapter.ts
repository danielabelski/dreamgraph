...
export class AnthropicAdapter implements ProviderAdapter {
  ...
  async callWithTools(request: ChatRequest): Promise<ChatResponse> {
    ...
    const data = (await res.json()) as AnthropicNonStreamResponse;
    ...
    const toolCalls: ToolCall[] = blocks
      .filter((b): b is AnthropicContentBlockToolUse => b.type === "tool_use")
      .map((b) => ({
        id: b.id,
        name: b.name,
        ...
        switch (evt.type) {
          ...
          case "content_block_delta": {
            ...
            if (dt === "text_delta" && typeof evt.delta?.text === "string") {
              ...
              if (buf) buf.jsonBuf += evt.delta.partial_json;
            }
            break;
          }
  ...
  private buildRequestBody(request: ChatRequest, stream: boolean): Record<string, unknown> {
    ...
    if (request.tools && request.tools.length > 0) {
      ...
      if (request.toolChoice) body.tool_choice = translateToolChoiceToAnthropic(request.toolChoice);
    }
...
function translateMessageToAnthropic(m: ChatMessage): Record<string, unknown> {
  if (m.role === "tool") {
    return {
      role: "user",
      content: [
...
function translateToolToAnthropic(t: ToolDefinition): Record<string, unknown> {
  return {
    name: t.name,
...
function translateToolChoiceToAnthropic(
  ...
  if (choice === "none") return { type: "auto" }; // Anthropic has no direct 'none'; closest is 'auto'.
  return { type: "tool", name: choice.name };
}

function mapStopReason(raw: string | undefined, hasToolCalls: boolean): ChatResponse["finishReason"] {
  switch (raw) {
    case "end_turn":
    case "stop_sequence":
