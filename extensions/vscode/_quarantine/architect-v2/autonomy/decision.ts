...
import { isTimeBudgetExhausted } from "./budget";
import type { CapabilityInventory } from "./capability";
import {
  createContinuationNeed,
...
export function filterByCapability(
  ...
  return candidates.filter((c) =>
    c.requiresCapabilities.every((cap) => inventory.has(cap)),
  );
}
 ...
 */
export function detectNoProgress(passes: readonly PassRecord[]): {
  noProgress: boolean;
  passesWithoutDelta: number;
} {
  if (passes.length < 2) return { noProgress: false, passesWithoutDelta: 0 };
  ...
  if (lastFp !== prevFp) return { noProgress: false, passesWithoutDelta: 0 };
  // Walk backwards to count how many consecutive passes share the fingerprint.
  let count = 2;
  for (let i = passes.length - 3; i >= 0; i--) {
    if (fingerprint(passes[i].endingArtifacts) === lastFp) count++;
    else break;
  }
...
function fingerprint(
  snapshots: readonly { id: string; hash: string }[],
): string {
...
export function buildContinuationPrompt(
  top: ActionCandidate,
  alternatives: readonly ActionCandidate[],
): string {
  const altList =
...
export function deriveNextAction(input: DeriveNextActionInput): DecisionResult {
  ...
  if (top.confidence < threshold) {
    return {
      ...
      ranked,
      note:
        `Top candidate confidence ${top.confidence.toFixed(2)} is below the ` +
