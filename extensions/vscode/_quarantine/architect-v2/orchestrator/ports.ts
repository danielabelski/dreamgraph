...
import type {
  ...
  DeclareRequirementsInput,
} from "../prompts/requirements.js";

// ---------------------------------------------------------------------------
// ContextBuilder port (8A.3)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Executor port (Slice 4 + 8A integration)
// ---------------------------------------------------------------------------
...
export interface ExecuteCapabilityInput {
  ...
  readonly invocationReason:
    | { kind: "user_action" }
    | { kind: "verification"; verificationKind: "build" | "test" | "type" | "lint" | "mcp_verify" | "invariant" }
    | { kind: "enrichment" };
}
...
// ---------------------------------------------------------------------------
// Clock port — pure, but injected so tests are deterministic
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Aggregate port bag passed into runPass
// ---------------------------------------------------------------------------
