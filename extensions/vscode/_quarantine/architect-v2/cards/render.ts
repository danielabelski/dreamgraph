...
import { assertNeverCard } from "./types.js";
 ...
 * stray separators.
 */
export function renderTrailingNote(note: string | undefined): string {
...
export function sanitizeFreeText(input: string): string {
  ...
  const fencedStripped = input.replace(
    /```[a-zA-Z0-9_+-]*\s*([\s\S]*?)```/g,
    (whole, body: string) => (looksLikeToolEnvelope(body.trim()) ? "" : whole),
  );
  ...
  for (const line of lines) {
    ...
    if (looksLikeToolEnvelope(trimmed)) continue;
    kept.push(line);
  }
 ...
 */
function stripBalancedToolEnvelopes(input: string): string {
  let out = "";
  let i = 0;
 ...
 */
function scanBalancedJson(s: string, start: number): number {
  const open = s[start];
...
function looksLikeToolEnvelope(payload: string): boolean {
  ...
  for (const item of items) {
    ...
    if (typeof type !== "string" || !ENVELOPE_TYPES.has(type)) return false;
  }
  return true;
}
...
function renderGoal(c: GoalCard): string {
  const lines: string[] = [];
  ...
  if (c.body.acceptance.length > 0) {
    ...
    for (const a of c.body.acceptance) lines.push(`- ${a}`);
  }
  return lines.join("\n");
}

function renderPlan(c: PlanCard): string {
  const lines: string[] = [];
...
function renderContext(c: ContextCard): string {
  const lines: string[] = [];
  ...
  if (c.body.sources.length > 0) {
    lines.push("");
    lines.push("**Sources:**");
    for (const a of c.body.sources) lines.push(`- ${formatArtifact(a)}`);
  }
  return lines.join("\n");
}

function renderDecision(c: DecisionCard): string {
  const lines: string[] = [];
...
function renderEdit(c: EditCard): string {
  const lines: string[] = [];
  ...
  if (c.body.artifacts.length > 0) {
    ...
    for (const a of c.body.artifacts) lines.push(`- ${formatArtifact(a)}`);
  }
  return lines.join("\n");
}

function renderVerification(c: VerificationCard): string {
  const lines: string[] = [];
...
function renderBlocker(c: BlockerCard): string {
  const lines: string[] = [];
  ...
  if (c.body.blockedBy.length > 0) {
    ...
    for (const b of c.body.blockedBy) lines.push(`- ${b}`);
  }
...
function renderNextStep(c: NextStepCard): string {
  const lines: string[] = [];
...
function renderCompletion(c: CompletionCard): string {
  const lines: string[] = [];
...
function renderFallback(c: FallbackCard): string {
  const lines: string[] = [];
...
function renderOutcome(c: OutcomeCard): string {
  const lines: string[] = [];
  ...
  lines.push(
    `**${o.kind.toUpperCase()}** \`${o.tool}\` (Tier ${o.tier}, ${o.intent})`,
  );
  ...
  if (o.kind === "success" || o.kind === "partial") {
    if (o.artifacts.length > 0) {
      lines.push("");
      lines.push("**Artifacts:**");
      for (const a of o.artifacts) lines.push(`- ${formatArtifact(a)}`);
    }
    if (o.kind === "partial") {
      lines.push("");
    ...
    lines.push(
      `_Recoverable: ${o.recoverable ? "yes" : "no"}${
        o.suggestedFallbackTier !== undefined
...
function headerLine(c: Card, title: string): string {
  return `### ${title} \`${c.id}\``;
}

function pillsLine(c: Card): string {
  const p = c.pills;
  const parts: string[] = [];
  ...
  if (p.certainty !== undefined) parts.push(`certainty:${p.certainty}`);
  if (p.mode !== undefined) parts.push(`mode:${p.mode}`);
  if (p.provider !== undefined) parts.push(`provider:${p.provider}`);
  if (p.graphBound !== undefined) parts.push(`graph:${p.graphBound}`);
  if (p.autonomyState !== undefined) parts.push(`status:${p.autonomyState}`);
  if (p.tier !== null && p.tier !== undefined) parts.push(`tier:T${p.tier}`);
...
function formatArtifact(a: ArtifactRef): string {
  return `[${a.kind}] \`${a.id}\``;
}
