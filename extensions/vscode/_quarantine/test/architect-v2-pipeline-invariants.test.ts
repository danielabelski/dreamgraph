...
import type { Card } from '../architect-v2/cards/types.js';
import { CARD_SCHEMA_VERSION } from '../architect-v2/cards/types.js';
import type { PillSet } from '../architect-v2/cards/pills.js';
import { CAPABILITY_MATRIX } from '../architect-v2/capabilities/matrix.js';
import { success } from '../architect-v2/execution/outcome.js';
import { richnessToGraphBound } from '../architect-v2/orchestrator/loop.js';
import { resolveCapabilityFromAction } from '../architect-v2/orchestrator/loop.js';
import { sanitizeProviderToolName } from '../architect-v2/execution/executor-adapter.js';

// ---------------------------------------------------------------------------
// I1 — graph-first mapping (locks the orchestrator's richness→pill seam)
// ---------------------------------------------------------------------------

test('I1: rich graph richness maps to graph:yes', () => {
  assert.equal(richnessToGraphBound('rich'), 'yes');
});

test('I1: partial / sparse richness map to graph:partial', () => {
  assert.equal(richnessToGraphBound('partial'), 'partial');
  assert.equal(richnessToGraphBound('sparse'), 'partial');
});

test('I1: absent or missing richness maps to graph:no (only fallback)', () => {
  assert.equal(richnessToGraphBound('absent'), 'no');
  assert.equal(richnessToGraphBound(undefined), 'no');
});
...
test('I2: sanitizer strips bare-line tool_use JSON array', () => {
  const input = [
  ...
  assert.ok(!/tool_use/.test(out), `tool_use leaked: ${out}`);
  assert.ok(out.includes('Starting with the highest-value read'));
  assert.ok(out.includes('Once I have these reads back'));
});

test('I2: sanitizer strips multi-line pretty-printed tool_use JSON', () => {
  const input = [
    'Plan:',
    '[',
    '  { "type": "tool_use", "id": "a1", "name": "read_file", "input": { "path": "package.json" } },',
  ...
  const out = sanitizeFreeText(input);
  assert.ok(!/tool_use/.test(out), `tool_use leaked: ${out}`);
  assert.ok(!/list_directory/.test(out), `tool name leaked: ${out}`);
  assert.ok(out.includes('Plan:'));
  assert.ok(out.includes('Then I will synthesize the report.'));
});

test('I2: sanitizer strips fenced tool_result JSON', () => {
  const input = [
    'Result:',
    '```json',
  ...
  const out = sanitizeFreeText(input);
  assert.ok(!/tool_result/.test(out), `tool_result leaked: ${out}`);
});

test('I2: sanitizer preserves prose containing only the word tool_use', () => {
...
test('I2: sanitizer leaves benign JSON objects alone', () => {
...
test('I2: renderPass routes free text through sanitizer', () => {
  const md = renderPass({
    cards: [],
    trailingNote:
      '[ { "type": "tool_use", "id": "x", "name": "noop", "input": {} } ]\nDone.',
  });
  assert.ok(!/tool_use/.test(md), `pass markdown leaked tool_use: ${md}`);
...
const REPRESENTATIVE_CARDS: ReadonlyArray<Card> = [
  {
    ...baseHeader('c-goal'),
    kind: 'goal',
...
test('I3: representative Card kinds render with an H3 header', () => {
  for (const c of REPRESENTATIVE_CARDS) {
    const md = renderCard(c);
    assert.ok(md.length > 0, `empty render for ${c.kind}`);
    assert.ok(/^### /.test(md), `missing H3 header for ${c.kind}: ${md.slice(0, 80)}`);
  }
});

test('I4: CAPABILITY_MATRIX exposes string ids (regression: Object.keys array bug)', () => {
  ...
  const ids = CAPABILITY_MATRIX.map((r) => r.id);
  assert.ok(ids.length > 30, `expected real capability count, got ${ids.length}`);
  for (const id of ids) {
    assert.ok(/[a-z]/.test(id), `id should be a string id, got ${id}`);
    assert.ok(!/^\d+$/.test(id), `id must not be a numeric index: ${id}`);
  }
});
...
test('I5: sanitizer rewrites characters outside the provider charset', () => {
  assert.equal(sanitizeProviderToolName('shell.run'), 'shell_run');
  assert.equal(sanitizeProviderToolName('mcp.read.file'), 'mcp_read_file');
  assert.equal(sanitizeProviderToolName('architect.reply'), 'architect_reply');
});

test('I5: sanitizer preserves already-safe names', () => {
  assert.equal(sanitizeProviderToolName('read_file'), 'read_file');
  assert.equal(sanitizeProviderToolName('list-tools'), 'list-tools');
  assert.equal(sanitizeProviderToolName('mcp_dreamgraph_query_resource'),
    'mcp_dreamgraph_query_resource');
});

test('I5: sanitizer output matches the strict provider regex', () => {
  ...
  for (const s of samples) {
    const out = sanitizeProviderToolName(s);
    assert.ok(re.test(out), `sanitized name '${out}' fails provider regex`);
  }
});
...
test('I6: every native tool descriptor resolves to a CapabilityId', async () => {
  const { NATIVE_TOOL_CATALOG } = await import(
    '../architect-v2/execution/catalog.js'
  );
  const unresolved: string[] = [];
  for (const desc of NATIVE_TOOL_CATALOG) {
    const cap = resolveCapabilityFromAction({
      id: 'tooluse_test',
      label: desc.tool,
      rationale: '',
      tool: desc.tool,
    ...
    if (!cap) unresolved.push(`${desc.tool} (intent=${desc.intent})`);
  }
  assert.equal(
    unresolved.length,
    0,
    `unresolved tool->capability mappings:\n  ${unresolved.join('\n  ')}`,
  );
});

test('I6: mcp_dreamgraph_cognitive_status resolves (specific blocker repro)', () => {
  const cap = resolveCapabilityFromAction({
    id: 'tooluse_repro',
  ...
  assert.ok(
    cap,
    'mcp_dreamgraph_cognitive_status must resolve to a CapabilityId; ' +
...
test('I8: provider tool schemas are valid across all transports', async () => {
  const { providerToolsForInventory } = await import(
    '../architect-v2/execution/executor-adapter.js'
  );
  const { NATIVE_TOOL_CATALOG } = await import(
    '../architect-v2/execution/catalog.js'
  );
  ...
  const inventory = { capabilities: new Set<string>() } as never;
  const bundle = providerToolsForInventory(
    resolved as never,
    inventory,
  );
  assert.ok(bundle.tools.length > 0, 'expected at least one tool emitted');
  for (const t of bundle.tools) {
    const s = t.inputSchema as Record<string, unknown>;
    assert.equal(s.type, 'object', `tool ${t.name} schema.type must be "object"`);
    assert.ok(
      s.properties && typeof s.properties === 'object',
      `tool ${t.name} schema.properties must be an object (OpenAI Responses requires this)`,
    );
    assert.notEqual(
      s.additionalProperties,
      true,
      `tool ${t.name} schema.additionalProperties must not be true (OpenAI strict mode rejects it)`,
    );
  }
});
