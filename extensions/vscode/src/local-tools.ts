...
export const LOCAL_TOOL_DEFINITIONS = [
  {
    name: 'run_command',
    description:
      '[Support tool — execution] Execute a shell command to build, test, or verify ' +
  ...
  },
  {
    name: 'modify_entity',
    description:
      '[Support tool — edit fallback] Replace a code entity in a file. Use as FALLBACK ' +
  ...
  },
  {
    ...
    description:
      '[Support tool — read fallback] Read a local file. Use only when MCP daemon is ' +
...
const LOCAL_TOOL_NAMES: Set<string> = new Set(LOCAL_TOOL_DEFINITIONS.map((t) => t.name));
...
function countMembers(text: string): number {
...
function getOutput(): vscode.OutputChannel {
  if (!_output) _output = vscode.window.createOutputChannel(OUTPUT_NAME, { log: true });
  return _output;
}
...
function keywordScore(line: string): number {
  ...
  let score = 0;
  if (/(error|fail|fatal|exception|traceback|npm err!|build failed)/.test(l)) score += 5;
  if (/(warning|warn)/.test(l)) score += 2;
  if (/(compiled|built|success|done|finished|passed)/.test(l)) score += 1;
  if (/^error\b|^warn\b/i.test(line)) score += 2;
  return score;
}

function extractRelevant(raw: string, maxLines = 60): string {
  ...
  for (const ln of picked) {
    if (ln.trim() === '') { blankRun++; if (blankRun <= 1) collapsed.push(ln); }
    else { blankRun = 0; collapsed.push(ln); }
  }
...
async function handleRunCommand(input: Record<string, unknown>): Promise<string> {
  const command = String(input.command ?? '');
  if (!command) return fail('command is required');

  const wsRoot = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
  if (!wsRoot) return fail('No workspace folder open');
...
async function handleModifyEntity(input: Record<string, unknown>): Promise<string> {
    ...
    try {
      ...
      if (target) {
        ...
        if (!parentEntity) {
          ...
          if (
            existingMembers > LARGE_ENTITY_MEMBER_THRESHOLD &&
            newMembers < existingMembers * 0.8
          ) {
            return fail(
              `Safety guard: "${entityName}" has ${existingMembers} members but the replacement ` +
...
async function regexEntityReplace(
 ...
 */
function findEntityEnd(text: string, startOffset: number): number {
  let depth = 0;
  let inString = false;
  let stringChar = '';
