...
export function deriveVerdict(content: string | undefined, trace: ToolTraceEntry[]): VerdictBanner {
  ...
  const terminalIncomplete =
    /\bpass aborted\b|\brequest failed before completion\b|\bcodex cli run was cancelled\b|\bwas cancelled by the host\b|\bcancelled by the host\b|\bcanceled by the host\b|\bdid not complete\b|\bdid not proceed\b|\bnot completed successfully\b|\bremains incomplete\b|\bassessment remains incomplete\b|\bprogress has stalled\b|\bstopped: progress has stalled\b|\byou've hit your usage limit\b|\busage limit\b|\brate limit\b|\bunsupported model\b|\bblocked by policy\b|\bcould not generate wrap-up summary\b|\btool call failed\b/.test(normalized);
  const explicitCompletion =
    /(?:^|\s)(?:verified:|confirmed:)|\bdone and verified\b|\bcompleted successfully\b|\bgoal sufficiently reached\b|\bready for commit\b/.test(normalized);
