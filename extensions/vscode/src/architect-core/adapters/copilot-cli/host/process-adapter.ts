...
import type {
  CopilotCliHelpResult,
  CopilotCliProcessPort,
  CopilotCliResolveResult,
  CopilotCliSpawnInput,
  CopilotCliSpawnResult,
...
async function resolveViaPowerShell(binaryName: string): Promise<string | null> {
  ...
  for (const c of candidates) {
    if (await isExecutableFile(c)) live.push(c);
  }
...
async function resolveOnPath(binaryName: string, env: Readonly<Record<string, string | undefined>>): Promise<string | null> {
  ...
  for (const dir of pathVar.split(delimiter)) {
    ...
    if (IS_WINDOWS) {
      ...
      for (const ext of WINDOWS_PATH_EXTS) {
        candidates.push(join(dir, binaryName + ext.toLowerCase()));
      }
      const lower = binaryName.toLowerCase();
      if (WINDOWS_PATH_EXTS.some((ext) => lower.endsWith(ext.toLowerCase()))) {
  ...
  if (IS_WINDOWS) {
    ...
    if (live.length === 0) return null;
    live.sort(
      (a, b) => rankWindowsShimByExtension(a) - rankWindowsShimByExtension(b),
    );
    return live[0];
  }
  for (const c of candidates) {
...
interface CaptureOptions {
  ...
  /**
   * Optional inactivity cap. When > 0, the run is killed if no
   * stdout/stderr chunk has been observed for this long; each chunk
   * resets the window. Distinct from `timeoutMs` (overall wall-clock
   * cap). When the run is killed by the idle timer, `timedOut` is
   * set to true so callers cannot tell them apart from a wall-clock
   * timeout — both mean "no further progress" from the host's
   * perspective.
   */
...
function captureRun(opts: CaptureOptions): Promise<CapturedRun> {
  return new Promise<CapturedRun>((resolve) => {
    ...
    const taskkillTreeOnWindows = (): void => {
      ...
      try {
        ...
        const k = spawn("taskkill.exe", ["/T", "/F", "/PID", String(pid)], {
          windowsHide: true,
          detached: true,
          stdio: "ignore",
        });
    ...
    const killHard = () => {
      if (sigtermTimer || sigkillTimer) return;
...
export const HOST_PROCESS: CopilotCliProcessPort = Object.freeze({
  async resolveExecutable(binaryName: string): Promise<CopilotCliResolveResult | null> {
  ...
  async spawn(input: CopilotCliSpawnInput): Promise<CopilotCliSpawnResult> {
