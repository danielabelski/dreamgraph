...
//
// Codex CLI adapter - orchestrator IO ports (Slice 3).
...
export interface CodexCliProcessPort {
  ...
  runRootHelp(input: { readonly command: string; readonly cwd: string; readonly env: Readonly<Record<string, string>> }): Promise<CodexCliCommandResult>;
  runExecHelp(input: { readonly command: string; readonly cwd: string; readonly env: Readonly<Record<string, string>> }): Promise<CodexCliCommandResult>;
  runLoginStatus(input: { readonly command: string; readonly cwd: string; readonly env: Readonly<Record<string, string>> }): Promise<CodexCliCommandResult>;
...
export interface CodexCliMcpAuditLivePort {
  ...
  ): Promise<CodexCliMcpAuditLiveSubscription>;
}
