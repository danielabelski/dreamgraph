 ...
 */
export const ARCHITECT_PASS_SCHEMA_NAME = 'architect_pass_envelope';
 ...
 */
export const ARCHITECT_PASS_JSON_SCHEMA = {
  type: 'object',
  ...
  properties: {
    narrative: {
      type: 'string',
      description:
        'Human-readable markdown body shown in the assistant chat bubble. May contain headings, lists, fenced code blocks, and inline code. Do NOT embed JSON envelopes here — the envelope fields are emitted separately.',
    },
    summary: {
      type: 'string',
      description:
        'One-line plain-text summary of what was completed in this pass. Shown on the SUMMARY card. Max ~200 chars.',
    },
    goal_status: {
      type: 'string',
      enum: ['complete', 'partial', 'blocked'],
      description:
        'complete = original goal sufficiently reached; partial = real progress but more to do; blocked = cannot proceed without input/tool/permission.',
    },
    progress_status: {
      type: 'string',
      enum: ['advancing', 'slowing', 'stalled'],
      description:
        'advancing = meaningful forward motion this pass; slowing = diminishing returns; stalled = no useful progress and unlikely to resume without a change.',
    },
    uncertainty: {
      type: 'string',
      enum: ['low', 'medium', 'high'],
      description:
        'Confidence in the conclusions stated above. high = significant unknowns or unverified assumptions remain.',
    },
    recommended_next_steps: {
      type: 'array',
      description:
        'Ordered list of concrete next actions. Empty when there is no safe next step (e.g. goal_status=complete or blocked awaiting user). Each entry becomes a clickable chip.',
      items: {
        type: 'object',
        additionalProperties: false,
        ...
        properties: {
          ...
          tool: {
            type: 'string',
            description:
              'Exact MCP/local tool name (snake_case) when the step maps to a single tool call. Empty string when the step is not a single-tool action.',
          },
          tool_args_json: {
            type: 'string',
            description:
              'JSON-encoded string of arguments for `tool`. Empty string when `tool` is empty. Must parse with JSON.parse when non-empty.',
          },
        },
      },
    },
 ...
 */
export function getArchitectPassSchemaInstructions(): string {
  return [
    '## Output contract',
 ...
 */
export function isArchitectPassEnvelope(value: unknown): value is ArchitectPassEnvelope {
  if (!value || typeof value !== 'object') return false;
  const v = value as Record<string, unknown>;
  if (typeof v.narrative !== 'string') return false;
  if (typeof v.summary !== 'string') return false;
  if (v.goal_status !== 'complete' && v.goal_status !== 'partial' && v.goal_status !== 'blocked') return false;
  if (v.progress_status !== 'advancing' && v.progress_status !== 'slowing' && v.progress_status !== 'stalled') return false;
  if (v.uncertainty !== 'low' && v.uncertainty !== 'medium' && v.uncertainty !== 'high') return false;
  if (!Array.isArray(v.recommended_next_steps)) return false;
  for (const step of v.recommended_next_steps) {
    ...
    if (typeof s.tool_args_json !== 'string') return false;
  }
  return true;
}
 ...
 */
export function tryParseArchitectPassEnvelope(content: string): ArchitectPassEnvelope | null {
  const trimmed = String(content ?? '').trim();
  if (!trimmed) return null;
  if (trimmed[0] !== '{') return null;
  try {
    const parsed = JSON.parse(trimmed);
    if (isArchitectPassEnvelope(parsed)) return parsed;
  } catch {
    // fall through
  }
  return null;
