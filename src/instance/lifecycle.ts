...
export async function createInstance(
  ...
  // engine.env — prefer selected global template, then repo template, then fall back to in-code default scaffold
  const templateName = opts.template ?? "default";
  const globalTemplateRoot = resolve(masterDir, "templates", templateName);
  const repoTemplateRoot = resolve(PROJECT_ROOT, "templates", templateName);
  const templateRoots = [globalTemplateRoot, repoTemplateRoot];
  const engineEnvTargetPath = resolve(instanceRoot, "config", "engine.env");
  let engineEnvFromTemplate = false;

  for (const templateRoot of templateRoots) {
    const engineEnvTemplatePath = resolve(templateRoot, "config", "engine.env");
    if (!existsSync(engineEnvTemplatePath)) {
      continue;
    }

    try {
      await copyFile(engineEnvTemplatePath, engineEnvTargetPath);
      engineEnvFromTemplate = true;
      logger.debug(`engine.env copied from ${engineEnvTemplatePath}`);
      break;
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      logger.warn(`engine.env template copy failed from ${engineEnvTemplatePath}, trying next source: ${msg}`);
    }
  }

  if (!engineEnvFromTemplate) {
    // Per ADR-051: the in-code fallback is an emergency stub, not a feature copy.
    // Rich, documented defaults live in templates/default/config/engine.env. If we got here
    // the template is missing from both the global and repo locations, which usually means
    // a broken install.
    const engineEnvContent = [
      "# DreamGraph engine.env (emergency fallback — broken install detected)",
      "# The full documented template was not found at templates/default/config/engine.env.",
      "# Reinstall or run `dg setup` to get the discoverable variable list.",
      "# DREAMGRAPH_LLM_PROVIDER=ollama",
      "",
    ].join("\n");

    await atomicWriteFile(engineEnvTargetPath, engineEnvContent);
    logger.warn(
      `engine.env: template not found at any known location — wrote minimal emergency stub. ` +
        `Tried: ${templateRoots.map((r) => resolve(r, "config", "engine.env")).join(", ")}. ` +
        `Reinstall DreamGraph to restore the full default template.`,
    );
  }
  ...
  if (!usedTemplates) {
    ...
    // Per ADR-051: the in-code DATA_STUBS are emergency-only. Loud warn signals
    // a broken install or missing templates/ tree.
    logger.warn(
      `Data stubs: templates/default/ not found at ${templateDir} — wrote minimal emergency stubs. ` +
        `Reinstall DreamGraph to restore the documented seed templates.`,
    );
  }
