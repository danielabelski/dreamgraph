...
function clusterCause(tension: TensionSignal): TensionClusterCause {
  ...
  if (/ungrounded|evidence|unknown entity|missing link/.test(text)) return "evidence_gap";
...
export function buildTensionClusters(tensions: TensionSignal[]): TensionCluster[] {
  const grouped = new Map<TensionClusterCause, TensionSignal[]>();
  for (const tension of tensions.filter((candidate) => !candidate.resolved)) {
    const cause = clusterCause(tension);
    grouped.set(cause, [...(grouped.get(cause) ?? []), tension]);
  }
  ...
  return [...grouped.entries()]
    ...
    .sort((left, right) => right.urgency_range.max - left.urgency_range.max || right.recurrence - left.recurrence || left.id.localeCompare(right.id));
}
