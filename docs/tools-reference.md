...
## Explorer HTTP Endpoints (v8 Phase 4)

The Explorer is an interactive single-page graph UI (Sigma.js + React) bundled to `dist/explorer-spa/` and served by the daemon at `/explorer/`. It complements the read-only dashboard with live graph navigation, server-sent events, and **curated mutations** that let an operator promote, reject, or resolve graph items with a written reason. Every mutation is appended to the Explorer Audit Log (`data/explorer_audit_log.jsonl`).
...
### POST Endpoints (Curated Mutations)

All mutation endpoints share the same precondition contract:

- Header `X-DreamGraph-Instance: <instance-uuid>` � must match the running daemon's instance UUID
- Header `If-Match: <etag>` � current snapshot etag; stale etag returns **HTTP 412 Precondition Failed**
- Body field `reason` � non-empty string, persisted in the audit log
- Returns `{ etag: <new-etag>, affected: string[] }` on success

| Route | Content-Type | Description |
|-------|--------------|-------------|
| `POST /explorer/mutations/tension.resolve` | `application/json` | Resolve an active tension. Body: `{ tension_id, reason }`. Marks the tension `resolved` and records the operator's rationale |
| `POST /explorer/mutations/candidate.promote` | `application/json` | Promote a candidate edge into the validated graph. Body: `{ candidate_id, reason }`. Writes through the standard promotion pipeline so confidence/decay rules still apply |
| `POST /explorer/mutations/candidate.reject` | `application/json` | Reject a candidate. Body: `{ candidate_id, reason }`. Removes the candidate from the pending pool and records the rationale |
| `POST /explorer/api/reason-suggest` | `application/json` | LLM-drafted reason text for the Resolve / Promote / Reject forms. Body: `{ intent: "tension.resolve" \| "candidate.promote" \| "candidate.reject", subject: { ... } }`. Uses the configured Dreamer LLM (currently `gpt-5.4-nano`). Response: `{ reason: string }`. Drafts are advisory � operators may edit before submitting the mutation |

### Design Principles

- **Optimistic concurrency** � Every mutation requires `If-Match: <etag>`; the SPA refetches the snapshot and surfaces a banner on 412 so the operator never overwrites a concurrent change.
